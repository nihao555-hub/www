...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`pages\` (
  	...
  	\`theme\` text DEFAULT 'electric-indigo',
  	\`published_at\` text,
  ...
  await db.run(sql`CREATE TABLE \`_pages_v\` (
  	...
  	\`version_theme\` text DEFAULT 'electric-indigo',
  ...
  await db.run(sql`CREATE TABLE \`ai_sites_images\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`image_id\` integer NOT NULL,
  	FOREIGN KEY (\`image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`ai_sites\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`ai_sites_images_order_idx\` ON \`ai_sites_images\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`ai_sites_images_parent_id_idx\` ON \`ai_sites_images\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`ai_sites_images_image_idx\` ON \`ai_sites_images\` (\`image_id\`);`)
  await db.run(sql`CREATE TABLE \`ai_sites\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`ai_sites_slug_idx\` ON \`ai_sites\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`ai_sites_updated_at_idx\` ON \`ai_sites\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`ai_sites_created_at_idx\` ON \`ai_sites\` (\`created_at\`);`)
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`categories_id\`) REFERENCES \`categories\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`ai_sites_id\`) REFERENCES \`ai_sites\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`users_id\`) REFERENCES \`users\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_ai_sites_id_idx\` ON \`payload_locked_documents_rels\` (\`ai_sites_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`ai_sites_images\`;`)
  await db.run(sql`DROP TABLE \`ai_sites\`;`)
